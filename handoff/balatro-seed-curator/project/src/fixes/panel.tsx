...
export function JimboBackButton({ onClick }: { onClick?: () => void }) {
  return (
    // No overflow:hidden here — let the text breathe
    <div style={{ display: 'flex', justifyContent: 'center', width: '100%', padding: '4px 0' }}>
      <button
        type="button"
        className="j-btn j-btn--orange j-btn--md j-btn--full"
        onClick={onClick}
        style={{ width: '100%', minWidth: 0 }}
      >
        {/* Explicit face div — not using j-back-btn class which may clip */}
        <div
        ...
        >
          <JimboText size="md">Back</JimboText>
        </div>
      </button>
    </div>
  )
