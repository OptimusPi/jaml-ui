...
function AppV2() {
  ...
  return (
    <DesignCanvas>
      <DCSection id="v2-analyzer" title="Jamlyzer" subtitle="Full-screen mobile modal · seed pager up top · single scrollable analyzer view per seed (all items, gold underglow on JAML hits) · full-width back at the bottom">
        <DCArtboard id="v2-analyzer-preview" label="Jamlyzer · seed 1/3" width={390} height={844}>
          <window.SeedAnalyzer seeds={window.SEEDS_V2} filter={window.FILTER_V2} onClose={() => {}} />
        </DCArtboard>
        <DCArtboard id="v2-analyzer-ante" label="Jamlyzer · scrolled" width={390} height={844}>
          <window.SeedAnalyzer seeds={window.SEEDS_V2} filter={window.FILTER_V2} onClose={() => {}} initialIdx={0} />
        </DCArtboard>
      </DCSection>
      <DCSection id="v2" title="Seed Detail v2" subtitle="Flat ante model · per-clause score columns · COD-style edge hit indicators">
        <DCArtboard id="v2-results-table" label="Results · sortable color-coded data table" width={390} height={844}>
          <window.SeedResultsTable seeds={window.SEEDS_V2} filter={window.FILTER_V2} />
        </DCArtboard>
      ...
      <DCSection id="v2-og" title="OG Card · /seed/[seed]/og.png" subtitle="1200×630 agnostic rollup — Discord unfurl, share link, embed">
        <DCArtboard id="v2-og-card" label="OG · winning seed" width={1200} height={630}>
          <SeedOGCard seed={window.SEEDS_V2[0]} />
        </DCArtboard>
      </DCSection>
