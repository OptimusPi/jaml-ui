...
(function(){
  ...
  function BalButton({
    ...
    // Balatro rule: hover and pressed are BOTH the same darker shade (the shadow color).
    const faceColor = (pressed || hover) ? sh : fg;
    ...
    return (
      ...
      >
        ...
        {/* button face — flat. NO border. NO edge. NO scale on hover. */}
        <div style={{
          position: 'relative',
          ...
          transition: 'transform 55ms linear, background-color 60ms linear',
          textAlign: 'center',
  ...
  function BackButton({ onClick }) {
    return (
      <div style={{ display: 'flex', justifyContent: 'center', width: '100%', padding: '8px 10px 10px' }}>
        <BalButton tone="orange" size="md" fullWidth onClick={onClick}>back</BalButton>
      </div>
    );
  }
