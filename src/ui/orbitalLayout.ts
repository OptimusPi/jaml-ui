// Orbit geometry for JimboOrbitalMenu. Pure math — no React, no DOM, no imports,
// so it is unit-testable and any renderer (DOM, SVG, canvas) can drive it.
//
// Harvested from @pifreak/jammy-orbital (`radialLayout.ts`), itself excavated
// from JAMMY. The math is unchanged; only the names moved into Jimbo's spelling.
//
...
//
//   1. Slots sit at *exactly even* angle steps. Items never drift to arbitrary
//      positions, so every open lands on the same lattice.
//   2. A pill that would overlap the mascot is pushed **outward along its own
//      ray** (k >= 1) — never sideways, never inward. Collision resolution
//      preserves the angle, so the lattice survives it.
//   3. Everything animates on one shared cubic-bezier(0.16, 1, 0.3, 1) — fast
//      out, long settle. Combined with (1), pills appear to snap into detents.
//
...
/** Angle reserved for the pinned back/start button. Positive Y is down. */
export const SOUTH_ANGLE_RAD = Math.PI / 2;

/** ~half height of an orbital pill (px) — paired with width for mascot clearance. */
export const PILL_HALF_H = 16;

/** Gap (px) between the mascot bitmap edge and the nearest pill corner. */
export const MASCOT_CLEARANCE_GAP = 10;

/** Floor width for the south button so it always reads as the primary action. */
...
/** Vertical breathing gap (px) between stacked pills. */
export const ARC_STACK_GAP = 6;

/** How far (px) the page-control centers sit above the south button's center. */
export const PAGE_CONTROL_LIFT = 22;

/** Distance below orbit center at which a boxless south pill is drawn. */
...
/** What the layout needs to know about a pill it has not measured. */
export interface OrbitMeasure {
  label: string;
  hasToggleDot: boolean;
}

/**
 * Estimated pill width for clamp math.
 *
 * Tuned for the pixel font at the `sm` button size plus its padding and the
 * optional toggle dot. Deliberately an estimate — measuring every pill would
 * force a layout pass per frame, and clearance only needs to be close.
 */
export function estimatePillWidth(label: string, hasToggleDot: boolean): number {
  const charW = 6.6;
  const basePad = 32;
  const dot = hasToggleDot ? 15 : 0;
  return Math.min(200, Math.max(56, basePad + dot + label.length * charW));
}

/**
 * Minimum distance from orbit center to pill center so the pill's *corner*
 * clears the round mascot bitmap. Uses diagonal reach, not half-width, so pills
 * at 45° don't clip.
 */
export function minDistanceFromMascotCenter(mascotHalfPx: number, pillHalfW: number): number {
  return mascotHalfPx + MASCOT_CLEARANCE_GAP + Math.hypot(pillHalfW, PILL_HALF_H);
}
 ...
 *
 * @param items        Orbital items — the south-pinned one must already be removed.
 ...
 * @param measure      Label + toggle-dot presence, for width estimation.
 */
export function layoutOrbitalEllipse<T>(
  ...
  return items.map((item, i) => {
    ...
    const minDist = minDistanceFromMascotCenter(mascotHalf, estimatePillWidth(label, hasToggleDot) / 2);

    // Degenerate center slot — push due east.
    if (d0 < 1e-4) return { item, x: minDist, y: 0 };
 ...
 *
 *   1. Every pill has the SAME HEIGHT. Only width varies, and it varies with
 *      content. So **height is the only parameter that governs overlap** —
 *      stack slots vertically with at least one pill-height between them and
 ...
 *   3. The arc is bounded by the TIPS of the extra-wide south button, which
 *      spans nearly the full width along the bottom — so the arc runs from its
 ...
 * mascot-clearance push that can exceed any radius bound (measured: a 103px
 * pill shoved 23px out of a 275x185 box). Height-spacing plus wall-flyout fills
 * the box exactly and cannot overflow it, because the wall IS the stop
 * condition — so the mascot never has to shrink to make the math work.
 *
 ...
 * tightest 275px-wide phone box, ~1px for ordinary pills. That is a cosmetic
 * cost paid knowingly: the character stays full-size and visible behind the
 * pill, while the alternative — pushing past the wall to clear him — clips the
 * pill out of the container, which is a functional bug. Never shrink the mascot
 * to buy clearance; that trade was made once already and rejected.
 *
 * @param southTopY Y of the topmost south chrome edge (the south button, or the
 *                  page controls when shown), relative to center, positive down.
 *                  The arc must not descend past it.
 */
export function layoutOrbitalArc<T>(
  ...
  // The band the arc may occupy: top wall down to the south chrome's top edge.
  const yTop = -halfH + PILL_HALF_H;
  const yBottom = Math.min(halfH - PILL_HALF_H, southTopY - PILL_HALF_H - ARC_STACK_GAP);

  // Two columns. Order runs up the left then down the right, so the sequence
  // still reads clockwise from the south button's left tip.
  const leftCount = Math.ceil(items.length / 2);
  const rightCount = items.length - leftCount;
  const step = 2 * PILL_HALF_H + ARC_STACK_GAP;

  // When a side holds more pills than the band fits at full spacing we compress
  // evenly rather than spilling: pills touching is a legible crowd, pills
  // outside the container is a bug.
  const slotY = (indexOnSide: number, countOnSide: number): number => {
    ...
    // Fill from the bottom tip upward, so a short menu hugs the south button
  ...
  return items.map((item, i) => {
    ...
    const { label, hasToggleDot } = measure(item);
    const pillHalfW = estimatePillWidth(label, hasToggleDot) / 2;
    ...
    return { item, x: side * Math.max(0, halfW - pillHalfW - SOUTH_EDGE_INSET), y };
  });
}

/**
 * How many pills a box holds on the arc without breaking the height law.
 *
 * The band conservatively reserves room for the page controls, so flipping from
 * "fits" to "paginates" can never itself shrink the band it was measured
 * against. This is the number a pager should respect: past it, show fewer items
 * per page — never smaller pills, and never a smaller mascot.
 */
export function arcPageCapacity(boxH: number): number {
  ...
  const yBottom = southButtonY - PAGE_CONTROL_LIFT - PILL_HALF_H - PILL_HALF_H - ARC_STACK_GAP;
  const step = 2 * PILL_HALF_H + ARC_STACK_GAP;
  return 2 * Math.max(1, Math.floor((yBottom - yTop) / step) + 1);
}

/**
 * Width of the south-pinned button when it rides the orbit (no box height).
 *
...
export function orbitRadiusForCount(mascotSizePx: number, orbitalCount: number, baseRadius: number): number {
  return Math.max(84, baseRadius + orbitalCount * 3, mascotSizePx / 2 + 42);
}

/**
 * Largest orbit radius that still fits inside a boxW x boxH container.
 *
 * The ring is centered in its box, so only half of each dimension is available,
 * and the outermost thing on the ring is not a pill *center* — it is the south
 ...
 *
 * Why this exists: orbitRadiusForCount grows the radius with item count from a
 * base derived from viewport WIDTH alone — height is never consulted. On a
 * short viewport that combination puts the south pill below the bottom edge
 * with no upper bound. Clamping to the box makes overflow structurally
 * impossible instead of something every host has to notice and hand-tune.
 */
export function maxOrbitRadiusForBox(boxW: number, boxH: number): number {
  return Math.max(0, Math.min(boxW, boxH) / 2 - PILL_HALF_H - SOUTH_PILL_OFFSET);
}

/**
 * Clamp a desired radius to its box.
 *
 * The floor keeps the ring clear of the mascot: squeezing below the mascot's
 * own half-size drops pills on top of the character, which reads as broken
 * rather than as tight. Below that point the box is simply too small for a
 * ring, and the caller should show fewer items rather than a smaller circle —
 * pagination already exists for exactly that.
 */
export function clampOrbitRadius(radius: number, mascotSizePx: number, maxRadius?: number): number {
  ...
  return Math.max(mascotSizePx / 2 + MASCOT_CLEARANCE_GAP, Math.min(radius, maxRadius));
}
