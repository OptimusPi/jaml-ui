...
const SHEET_ASPECTS = {
  ...
  enhancers: 95 / 71,
  stickers:  95 / 71,
  decks:     95 / 71,    // 8BitDeck card aspect
  stakes:    1,           // 29×29 chips
};

// ─── Deck name → {x,y} on 8BitDeck.png. Col 12 = Ace, rows are suits.
//     Balatro deck names mapped to thematic suits.
const DECK_POS = {
  red:        { x: 12, y: 0 }, // Hearts ace
  blue:       { x: 12, y: 2 }, // Diamonds ace
  yellow:     { x: 12, y: 1 }, // Clubs ace (gold-ish)
  green:      { x: 12, y: 1 }, // Clubs ace
  black:      { x: 12, y: 3 }, // Spades ace
  magic:      { x: 12, y: 0 },
  nebula:     { x: 12, y: 3 },
  ghost:      { x: 12, y: 3 },
  abandoned:  { x: 12, y: 3 },
  checkered:  { x: 11, y: 0 }, // King of hearts
  zodiac:     { x: 12, y: 1 },
  painted:    { x: 12, y: 2 },
  anaglyph:   { x: 12, y: 0 },
  plasma:     { x: 12, y: 2 },
  erratic:    { x: 11, y: 3 }, // King of spades — chaotic
};
SPRITE_MAPS.decks = Object.fromEntries(
  Object.entries(DECK_POS).map(([k, v]) => [k, v])
);

// ─── Stake name → {x,y} on balatro-stake-chips.png (5×2).
const STAKE_POS = {
  white:  { x: 0, y: 0 },
  red:    { x: 1, y: 0 },
  green:  { x: 2, y: 0 },
...
SPRITE_MAPS.stakes = Object.fromEntries(
  Object.entries(STAKE_POS).map(([k, v]) => [k, v])
);

// ─── <DeckSprite> — one Balatro deck card. Accepts deck name like "Red"
//     (resolves to "red" → Ace of Hearts on the 8BitDeck sheet).
function DeckSprite({ name, size = 36, style = {} }) {
  const w = size;
  const h = Math.round(size * 95 / 71);
  // Strip "Deck" suffix if present
  const k = String(name || '').replace(/\s*deck\s*$/i, '').trim();
  return (
    <Sprite sheet="decks" name={k} width={w} height={h} style={{
      filter: 'drop-shadow(1px 2px 0 rgba(0,0,0,.7))',
      ...style,
    }} />
  );
}

// ─── <StakeSprite> — small 29×29 stake chip. Accepts "Gold", "white", etc.
function StakeSprite({ name, size = 22, style = {} }) {
  const k = String(name || '').replace(/\s*stake\s*$/i, '').trim();
  return (
    <Sprite sheet="stakes" name={k} width={size} height={size} style={{
...
window.DeckSprite = DeckSprite;
window.StakeSprite = StakeSprite;
