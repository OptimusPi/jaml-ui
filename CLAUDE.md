...
6. **`src/components/GameCard.tsx`** — composes the layer stack for a specific item (`JamlGameCard`, `JamlVoucher`, `JamlTag`, `JamlBoss`), combining base sprite + edition + stickers + seal/enhancer for standard cards. Also exposes `resolveAnalyzerShopItem` which turns an analyzer `{id,name}` into the appropriate renderable shape.
...
- **`src/decode/packedBalatroItem.ts`** (exported via `core`) — pure bit-math for the Balatro/analyzer shop encoding: top nibble = category (`Standardcard=1 … Joker=5, Invalid=0xf`), next two bits = joker rarity, low bits = index. No runtime dependencies.
